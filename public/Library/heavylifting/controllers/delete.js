...
//---------------------------------------------------------------------------------------------------------------------------
 res.locals.models[res.locals.collections[res.locals.routeSet]['datacollectionname']].findOneAndUpdate({
	  $and : 
	  [
	  {$or: [
	    {"elementID": idItem },
...
//find and review the document permissions.
	query1.exec(function (err, item) {
	  if (err) { return next(err); }
		...
		if (req.user.id == item[0].userID | req.user.permission == 'administrator' | req.user.permission == 'superadmin') {
//---------------------------------------------------------------------------------------------------------------------------
  res.locals.models[res.locals.collections[res.locals.routeSet]['collectionName']].findOneAndRemove({
	  $and : 
	  [
	  {$or: [
	    {"elementID": idItem },
